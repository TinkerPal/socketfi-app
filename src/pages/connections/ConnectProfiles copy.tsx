...
export default function ConnectProfiles() {
  ...
  useEffect(() => {
    async function refreshUserSession() {
      ...
      try {
        const res = await getRequest("user", "", accessToken);
    ...
    refreshUserSession();
  }, [accessToken, setActiveSession, updateData]);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);

    if (params.get("twitter") !== "pending_onchain") return;

    if (twitterOptionsFetchedRef.current) return;
    twitterOptionsFetchedRef.current = true;

    async function loadTwitterLinkOptions() {
      try {
        const res = await fetch(`${BASE_URL}/auth/twitter/link/options`, {
          credentials: "include",
        });
        ...
        setTwitterPendingLink(data);

        setModalInfo({
          title: "Confirm X Account",
          platform: "twitter-onchain",
          description:
            "This X account was authenticated successfully. Review it before mapping it to your smart wallet.",
          isOpen: true,
          inputType: "confirm",
        ...
        console.error(err);
        toast.error(err.message || "Failed to load Twitter link options.");
      }
    }

    loadTwitterLinkOptions();
  }, []);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);

    if (params.get("discord") !== "pending_onchain") return;

    if (discordOptionsFetchedRef.current) return;
    discordOptionsFetchedRef.current = true;

    async function loadDiscordLinkOptions() {
      try {
        const res = await fetch(`${BASE_URL}/auth/discord/link/options`, {
          credentials: "include",
        });
        ...
        setDiscordPendingLink(data);

        setModalInfo({
          title: "Confirm Discord Account",
          platform: "discord-onchain",
          description:
            "This discord account was authenticated successfully. Review it before mapping it to your smart wallet.",
          isOpen: true,
          inputType: "confirm",
        ...
        console.error(err);
        toast.error(err.message || "Failed to load Discord link options.");
      }
    }

    loadDiscordLinkOptions();
  }, []);
  ...
  useEffect(() => {
    ...
    if (twitter === "failed") {
      toast.error(decodeURIComponent(error || "Twitter connection failed"));
  ...
  useEffect(() => {
    ...
    if (discord === "failed") {
      toast.error(decodeURIComponent(error || "Discord connection failed"));
